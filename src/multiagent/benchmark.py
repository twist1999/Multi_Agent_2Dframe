...
def generate_benchmark_cases(count: int = 30, seed: int = 20260506) -> list[BenchmarkCase]:
    rng = random.Random(seed)
    batch_id = datetime.now(timezone.utc).strftime("batch-%Y%m%d-%H%M%S")
    cases: list[BenchmarkCase] = []
    for index in range(1, count + 1):
        bay_count = rng.randint(2, 5)
        bay_lengths = [rng.choice([4.5, 5.0, 6.0, 7.5, 8.0]) for _ in range(bay_count)]
        story_counts = [rng.randint(2, 5) for _ in range(bay_count)]
        story_height_pool = [3.6, 4.0, 4.5, 5.0, 5.5, 6.0, 7.0]
        shared_story_heights = [
            rng.choice(story_height_pool)
            for _ in range(max(story_counts))
        ]
        story_heights = [shared_story_heights[:story_count] for story_count in story_counts]
        uniform_load = rng.choice([5, 8, 10, 12, 15, 18, 20, 25])
        node_force = rng.choice([20, 30, 40, 50, 60, 75, 90, 100])
        ...
        cases.append(BenchmarkCase(case_id=f"{batch_id}-case-{index:02d}", batch_id=batch_id, prompt=prompt))
    return cases
